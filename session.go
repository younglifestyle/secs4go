...
func NewSessionWithId(codec Codec, sendChanSize int, id uint64) *Session {
	return newSessionWithId(nil, codec, sendChanSize, id)
}

func newSessionWithId(manager *Manager, codec Codec, sendChanSize int, id uint64) *Session {
	session := &Session{
		codec:     codec,
		manager:   manager,
		closeChan: make(chan int),
