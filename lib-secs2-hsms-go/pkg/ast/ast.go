...
func (node *DataMessage) GetAscii(indices ...int) (string, error) {
	itemNode, err := node.dataItem.Get(indices...)
	if err != nil {
		return "", err
	}

	if itemNode.Type() == "ascii" {
		return itemNode.Values().(string), nil
	} else {
		return "", fmt.Errorf("is %s, not ascii", itemNode.Type())
	}

	// not reachable
}

func (node *DataMessage) GetByte(indices ...int) (byte, error) {
	itemNode, err := node.dataItem.Get(indices[:len(indices)-1]...)
	if err != nil {
		return 0, err
	}

	if itemNode.Type() == "binary" {
		ints := itemNode.Values().([]int)
		return byte(ints[indices[len(indices)-1]]), nil
	} else {
		return 0, fmt.Errorf("is %s, not binary", itemNode.Type())
	}

	// not reachable
}

func (node *DataMessage) GetBoolean(indices ...int) (bool, error) {
	itemNode, err := node.dataItem.Get(indices[:len(indices)-1]...)
	if err != nil {
		return false, err
	}
	...
	if itemNode.Type() == "boolean" {
		data := itemNode.Values().([]bool)
		return data[indices[len(indices)-1]], nil
	} else {
		return false, fmt.Errorf("is %s, not boolean", itemNode.Type())
	}

	// not reachable
}

func (node *DataMessage) GetFloat(indices ...int) (float64, error) {
	itemNode, err := node.dataItem.Get(indices[:len(indices)-1]...)
	if err != nil {
		return 0, err
	}

	if itemNode.Type()[:1] == "f" {
		data := itemNode.Values().([]float64)
		return data[indices[len(indices)-1]], nil
	} else {
		return 0, fmt.Errorf("is %s, not float", itemNode.Type())
	}

	// not reachable
}

// GetInt indices必须传入数值，但函数中不加以验证，传错，以panic告警
func (node *DataMessage) GetInt(indices ...int) (int64, error) {
	itemNode, err := node.dataItem.Get(indices[:len(indices)-1]...)
	if err != nil {
		return 0, err
	}

	if itemNode.Type()[:1] == "i" {
		// direct get
		ints := itemNode.Values().([]int64)
		return ints[indices[len(indices)-1]], nil
	} else {
		return 0, fmt.Errorf("is %s, not int", itemNode.Type())
	}

	// not reachable
}

func (node *DataMessage) Get(indices ...int) (ItemNode, error) {

	return node.dataItem.Get(indices...)
}
