...
func (node emptyItemNode) Values() interface{} {
	return ""
}
...
func (node emptyItemNode) Get(indices ...int) (ItemNode, error) {
	if len(indices) == 0 {
		return emptyItemNode{}, nil
	} else {
		return nil, fmt.Errorf("not list, is %s", node)
	}
}
